...
// root-level FlyToBagLayer animates it to the bag capsule in the dock, then reports the
// landing so the capsule can bump on arrival rather than the instant the store
...
interface FlyToBagState {
  flights: Flight[]
  /** Bumps once per landing; CartCapsule reacts to the change. */
  landed: number
