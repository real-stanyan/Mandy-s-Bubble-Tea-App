# mandys_bubble_tea_app

React Native (Expo) mobile app for Mandy's Bubble Tea — native counterpart to the Next.js web site, sharing the same Square backend (Catalog, Orders, Payments, Loyalty, Customers). Done = feature works in the app with the gate green. Scope: app only — server-side code lives in the web repo's API routes, never here.
...
Expo SDK 54 / Expo Router 6 (file-based routing) / TypeScript
React Native core components + @expo/vector-icons / react-native-reanimated 4.x / expo-image
Zustand (cart, persisted to AsyncStorage) / Supabase JS client
Square backend via `lib/api.ts` → existing Next.js API routes (the app runs no server code)
EAS Build + OTA updates (expo-updates); `ios/` folder is hand-curated — no `expo prebuild`
Jest for tests

## Hard rules

Always use `serializeSquareResponse()` when handling Square API data — BigInt breaks JSON.
All Square money amounts are **cents as number** (BigInt converted server-side).
API calls go through `lib/api.ts` to the existing Next.js API routes — the app never runs server-side code.
Env vars needed in the app use the `EXPO_PUBLIC_` prefix; never ship secrets (e.g. Supabase service-role key) into the client bundle.
StyleSheet or NativeWind for styling — no web-only CSS.
`ios/` is hand-curated: never run `expo prebuild`; adding native libs = hand-edit Info.plist; version bumps must sync app.json and Info.plist together.
...
- **Who merges**: the PR's author agent merges it themself once CI is green. Protocol changes follow the tier system (see "Changing the protocol itself"): L1 waits for `real-stanyan` agreement, L2 is autonomous.
- **A second agent's review is not mandatory**: in serial repos only one shift is present at a time, and forcing mutual review would block at handoff boundaries; parallel lanes (ADR-0048) don't change this — review stays optional, because the quality backstop never depended on serialization: the CI gate + the `real-stanyan`'s after-the-fact veto (revert + reopen the issue), plus branch protection where configured (ADR-0042).
- **Don't take over someone else's open PR** — that's a mid-task handoff (see While working). Exception: the handoff issue explicitly transfers it, or the `real-stanyan` directs it.
...
|---|---|---|
| **L1 strict tier** | Hard rules / Gate command / Tech stack / this section itself | issue + ADR + PR, **and the agent may only merge after the `real-stanyan` explicitly agrees, in the session or in a PR comment** |
...
L1's "explicit agreement" is a weak-b form: it's enough for the `real-stanyan` to say "agreed" in the session or write "agreed" in a PR comment, and the agent presses the merge button itself. **For the PR-comment path, only a comment authored by the GitHub account `real-stanyan` names counts (ADR-0034)** — anyone else's "agreed" is not L1 approval. **In a repo with more than one human collaborator, only the PR-comment path is valid L1 approval (ADR-0042)** — in-session agreement stops counting (including in the maintainer's own session): in-session approval leaves no verifiable trace, so a merged L1 PR without the maintainer's comment would be indistinguishable from an impersonated approval. Single-human repos keep both paths. **GitHub's Approve button is not required** — the cost is that the `real-stanyan` becomes the L1 bottleneck, and that cost is accepted.
...
- A stalled lane is released by the `real-stanyan`: unassign its tasks, close its handoff (the stale-claim rule in ADR-0047 already makes dangling assignments non-binding).
...
No fixed division of labor (ADR-0008 option 2): default rule = **Task-issue claim-based ownership** — whoever claims a task sees it through start to finish (see While working); tasks aren't routed by agent specialty.
...
- `.claude/*.md` — module docs, read before working on each area: `square-api.md`, `catalog.md`, `cart-checkout.md`, `payment.md`, `loyalty.md`, `account.md`, `deployment.md`, `architecture.md` (project structure + web-vs-app differences)
